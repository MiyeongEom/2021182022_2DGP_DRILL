...
class WalkingState:
    ...
    def draw(boy):
        sx, sy = boy.x - server.background.window_left, \
                 boy.y - server.background.window_bottom

        boy.font.draw(sx - 80, sy + 70, 'score : %d' % (boy.score), (255, 255, 0))
...
class Boy:
    ...
    def get_bb(self):
        return self.x - server.background.window_left - 20, \
               self.y - server.background.window_bottom - 40, \
